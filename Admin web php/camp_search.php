		...
		<form method="post" action="camp_search.php">	
			<div class="input-group mb-3">
				<select class="btn btn-outline-secondary dropdown-toggle" name="s_kind">
					...
					<option value="name" <? if($s_kind == "name"){ ?> selected <? } ?>>이름</option>
					<option value="address" <? if($s_kind == "address"){ ?> selected <? } ?>>주소</option>
                    <option value="etc" <? if($s_kind == "etc"){ ?> selected <? } ?>>기타 정보</option>
				</select>
		...
		<div class="table-responsive">
			<table class="table table-striped table-sm">
                ...
                <?
					...
					if($s_kind=="all"){
                        $sql = "SELECT *  FROM camp_all where name like '$s_text2' || address like '$s_text2' || etc like '$s_text2' || lat like '$s_text2' || lon like '$s_text2' || camping_kind like '$s_text2'";
                    }
                    else {
                        $sql = "SELECT *  FROM camp_all where $s_kind like '$s_text2'";
                    }
